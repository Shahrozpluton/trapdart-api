...
const updatePicture = async (req, res) => {
    try {
        ...
        const record = await pool.query("SELECT * from picture");

        const picture = record.rowCount === 0
            ? await pool.query("INSERT INTO picture (image_url) VALUES($1) RETURNING *",[image])
            : await pool.query("UPDATE picture SET image_url = $1 WHERE id = 1",[image]);
